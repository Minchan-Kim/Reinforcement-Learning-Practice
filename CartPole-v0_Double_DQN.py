...
class Double_DQN:
    ...
    def learn(self):
        ...
        for i in range(self.batch_size):
            ...
            if not done:
                next_action = np.argmax(self.model.predict(np.array([next_state]), batch_size = 1)[0])
                target_data[i, action] = reward + self.discount_rate * (self.target_model.predict(np.array([next_state]), batch_size = 1)[0][next_action])
            else:
